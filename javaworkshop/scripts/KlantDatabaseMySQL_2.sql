...
CREATE TABLE IF NOT EXISTS `mydb`.`ADRES` (
  `adres_id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '', 
  ...
  UNIQUE INDEX `adres_id_UNIQUE` (`adres_id` ASC)  COMMENT '')
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`KLANT_ADRES` (
  ...
  `adres_id` INT UNSIGNED NOT NULL COMMENT '',
  PRIMARY KEY (`klant_id`, `adres_id`)  COMMENT '',
  UNIQUE INDEX `id_combo_UNIQUE` (`klant_id` ASC, `adres_id` ASC)  COMMENT '',
  CONSTRAINT `klant_id_adres`
...
CREATE TABLE IF NOT EXISTS `mydb`.`BESTELLING` (
  ...
  CONSTRAINT `klant_id_bestelling`
    FOREIGN KEY (`klant_id`)
    REFERENCES `mydb`.`KLANT` (`klant_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
